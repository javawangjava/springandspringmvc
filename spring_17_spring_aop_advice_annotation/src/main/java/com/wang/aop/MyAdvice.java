...
public class MyAdvice {
    ...
    @Pointcut("execution(* com.wang.dao.AccountDao.myMethod2(..) )")
    private void myPointcut2() {  // 切入点方法
    }

    @Pointcut("execution(* com.wang.dao.AccountDao.myMethod3(..) )")
    private void myPointcut3() {  // 切入点方法
    }

    @Pointcut("execution(* com.wang.dao.AccountDao.myMethod4(..) )")
    private void myPointcut4() {  // 切入点方法
    }

    @Pointcut("execution(* com.wang.dao.AccountDao.myMethod5(..) )")
    private void myPointcut5() {  // 切入点方法
    }
    ...
    // public void myMethod1()的无返回值，这里将Around方法的返回值设置为Object类型，最后返回null
    @Around("myPointcut1()")
    public Object myAround1(ProceedingJoinPoint pjp) throws Throwable {//由于无法预知原始方法运行后是否会抛出异常，因此环绕通知方法必须抛出Throwable对象
        System.out.println("around before myPointcut1....");
        // 使用ProceedingJoinPoint对象的proceed()方法表示对原始对象方法进行调用，返回值为原始对象方法的返回值。
        pjp.proceed();
        System.out.println("around after myPointcut1...");
        return null;
    }
 ...
 /*
    ...
    public void myBefore() {
        System.out.println("myPointcut2运行之前，myBefore运行了");
    }
*/
    ...
    @After("myPointcut3()")
    public void myAfter() {
        System.out.println("myPointcut3运行之后，myAfter运行了");
    }
    */
 ...
 /*
    ...
    public void myAfterReturning() {
        System.out.println("myPointcut4执行之后没有异常，myAfterReturning运行了");
    }
    */
    ...
    /*
    //  抛出异常后通知@AfterThrowing(了解)：在切入点方法【匹配的方法】执行之后执行，只有当切入点方法内部出现异常之后才执行。
    @AfterThrowing("myPointcut5()")
    public void myAfterThrowing() {
        System.out.println("myPointcut5执行中抛出异常后，myAfterThrowing运行了");
    }
    */
